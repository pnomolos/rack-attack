...
pub struct QueryData<'a> {
    raw: &'a str,
    parsed: RefCell<Option<HashMap<String, String>>>,
}
...
impl<'a> QueryData<'a> {
    ...
    pub fn get(&self, key: &str) -> Option<String> {
        self.ensure_parsed();
        self.parsed.borrow().as_ref().unwrap().get(key).cloned()
    }
    ...
    fn ensure_parsed(&self) {
        ...
        let map: HashMap<String, String> = self
            ...
            .collect();
        *self.parsed.borrow_mut() = Some(map);
    }
}
...
mod tests {
    ...
    fn test_basic_query_parsing() {
        ...
        assert_eq!(qd.get("q"), Some("hello".to_string()));
        assert_eq!(qd.get("page"), Some("1".to_string()));
        assert_eq!(qd.get("sort"), Some("asc".to_string()));
    ...
    fn test_key_without_value() {
        ...
        assert_eq!(qd.get("flag"), Some("".to_string()));
        assert_eq!(qd.get("key"), Some("val".to_string()));
    }
    ...
    fn test_duplicate_keys_last_wins() {
        ...
        assert_eq!(qd.get("a"), Some("2".to_string()));
    }
    ...
    fn test_equals_in_value() {
        ...
        assert_eq!(qd.get("key"), Some("a=b".to_string()));
    }
    ...
    fn test_empty_key() {
        ...
        assert_eq!(qd.get(""), Some("value".to_string()));
    }

    #[test]
    fn test_url_encoded_values() {
        let qd = QueryData::new("name=hello%20world&path=%2Ffoo%2Fbar");
        assert_eq!(qd.get("name"), Some("hello world".to_string()));
        assert_eq!(qd.get("path"), Some("/foo/bar".to_string()));
    }

    #[test]
    fn test_url_encoded_keys() {
        let qd = QueryData::new("my%20key=value");
        assert_eq!(qd.get("my key"), Some("value".to_string()));
    }

    #[test]
    fn test_plus_as_space() {
        let qd = QueryData::new("q=hello+world");
        assert_eq!(qd.get("q"), Some("hello world".to_string()));
    }
}
